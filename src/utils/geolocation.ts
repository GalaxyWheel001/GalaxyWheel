...
export async function detectUserLocation(): Promise<GeolocationData> {
  // Проверяем кэш
  const cached = localStorage.getItem('galaxy_wheel_location_cache');
  if (cached) {
    try {
  ...
  try {
    ...
    if (response.data && response.data.country_code) {
      ...
      // Сохраняем в кэш
      localStorage.setItem('galaxy_wheel_location_cache', JSON.stringify({
        data: locationData,
        timestamp: Date.now()
      }));
...
function getCurrencyByCountry(countryCode: string): SupportedCurrency {
  const currencyMap: Record<string, SupportedCurrency> = {
    ...
    'SK': 'SKK', 'SI': 'SIT', 'EE': 'EEK', 'LV': 'LVL', 'LT': 'LTL', 'MT': 'MTL',
    'CY': 'CYP', 'LU': 'EUR', 'DK': 'DKK', 'NO': 'NOK', 'SE': 'SEK', 'CH': 'CHF',
    ...
    'AZ': 'AZN', 'KZ': 'KZT', 'UZ': 'UZS', 'KG': 'KGS', 'TJ': 'TJS', 'TM': 'TMT',
    'RU': 'RUB',
    ...
    'NP': 'NPR', 'BT': 'BTN', 'MV': 'MVR', 'IN': 'INR', 'PK': 'PKR', 'AF': 'AFN',
    'IR': 'IRR', 'IQ': 'IQD', 'SY': 'SYP', 'LB': 'LBP', 'JO': 'JOD', 'PS': 'ILS',
    'IL': 'ILS', 'SA': 'SAR', 'AE': 'AED', 'QA': 'QAR', 'KW': 'KWD', 'BH': 'BHD',
    'OM': 'OMR', 'YE': 'YER', 'QA': 'QAR', 'BH': 'BHD', 'OM': 'OMR', 'YE': 'YER',
    ...
    'AO': 'AOA', 'MZ': 'MZN', 'ZW': 'ZWL', 'ZA': 'ZAR', 'MG': 'MGA', 'MU': 'MUR',
    ...
    'ST': 'STD', 'AO': 'AOA', 'MZ': 'MZN', 'ZW': 'ZWL', 'ZA': 'ZAR', 'MG': 'MGA',
    ...
    'NU': 'NZD', 'TK': 'NZD', 'AS': 'USD', 'GU': 'USD', 'MP': 'USD', 'PW': 'USD',
    'FM': 'USD', 'MH': 'USD', 'CK': 'NZD', 'NU': 'NZD', 'TK': 'NZD', 'AS': 'USD',
    'GU': 'USD', 'MP': 'USD',
